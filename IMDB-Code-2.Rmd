...
```{r}
DONTUSE = read.csv("data_final.csv")
attach(imdb)
```
...
comboS = glm(imdb_score~poly(duration_mins,2)+director_column10+action+thriller+romance+horror+drama+movie_budget+movie_meter_IMDB_pro+animation+distributor10+number_of_faces_in_movie_poster+poly(ratio_movie_cast_likes,2)+sum_total_likes+poly(movie_facebook_likes,2)+color+actor_1_star_meter+musical+content10, data=imdb)

cv.glm(imdb, comboS, K=46)$delta[1] 
summary(comboS)
...
comboS.lm = lm(imdb_score~poly(duration_mins,2)+director_column10+action+thriller+romance+horror+drama+movie_budget+movie_meter_IMDB_pro+animation+distributor10+number_of_faces_in_movie_poster+poly(ratio_movie_cast_likes,2)+sum_total_likes+poly(movie_facebook_likes,2)+color+actor_1_star_meter+musical+content10, data=imdb)
ncvTest(comboS.lm)
plot(predict(comboS), residuals(comboS), col="red")
abline(0,0,lty = 2)
library(sandwich)
library(lmtest)
library(plm)
coeftest(comboS, vcov. = vcovHC(comboS, type = "HC1"))
summary(comboS.lm)

########## collinarity test ###########
install.packages("olsrr")
library(olsrr)
ols_coll_diag(comboS) #sum total likes & cast total likes have collinearity so we remove one.. cast total likes.

```
Predictions
```{r}
coef(comboS.lm)[1]+105*coef(comboS.lm)[2]+"Yarrow Cheney"*coef(comboS.lm)

testing123=lm(imdb_score~poly(duration_mins,2),data = imdb)
summary(testing123)

coef(testing123)[1]+105*coef(testing123)[2]+105*coef(testing123)[3]

##########################
newest = glm(imdb_score~poly(duration_mins,2)+director_column10+action+thriller+romance+horror+drama+animation+distributor10+number_of_faces_in_movie_poster+poly(movie_facebook_likes,2)+color+musical+content10, data=imdb)
cv.glm(imdb, newest, K=52)$delta[1] #MSE= .72413
summary(newest)

#Predicting The Grinch 
predict(newest, data.frame(duration_mins=105, director_column10="Other", action = 0, thriller=0,romance=0,horror=0,drama = 0, animation=1,distributor10="Universal Pictures",number_of_faces_in_movie_poster=2, movie_facebook_likes=574,000, color="Color",musical=0,content10="PG")) # Rating = 6.75517

#Predicting Postcards From London
predict(newest, data.frame(duration_mins=90, director_column10="Other", action = 0, thriller=0,romance=0,horror=0,drama = 1, animation=0, distributor10="Other",number_of_faces_in_movie_poster=5, movie_facebook_likes=0,color="Color",musical=0,content10="Other")) # Rating = 6.4542

#Predicting The Long Dumb Road
predict(newest, data.frame(duration_mins=90, director_column10="Other", action = 0, thriller=0,romance=0,horror=0,drama =0, animation=0, distributor10="Universal Pictures",number_of_faces_in_movie_poster=2, movie_facebook_likes=111,
color="Color",musical=0,content10="R")) #Rating = 6.16885

#Predicting Instant Family
predict(newest, data.frame(duration_mins=90, director_column10="Other", action = 0, thriller=0,romance=0,horror=0,drama =0, animation=0, distributor10="Paramount Pictures",number_of_faces_in_movie_poster=5, movie_facebook_likes=35000,
color="Color",musical=0,content10="PG-13")) # Rating = 6.356787

#Predicting Crimes of Grindelwald
predict(newest, data.frame(duration_mins=134, director_column10="Other", action = 0, thriller=0,romance=0,horror=0,drama = 1, animation=0, distributor10="Warner Bros.",number_of_faces_in_movie_poster=10, movie_facebook_likes=6500,
color="Color",musical=0,content10="PG-13")) #Rating = 6.518825

#Predicting The Clovehitch Killer
predict(newest, data.frame(duration_mins=110, director_column10="Other", action = 0, thriller=0,romance=0,horror=1,drama = 1, animation=0, distributor10="Other",number_of_faces_in_movie_poster=1, movie_facebook_likes=0,
color="Color",musical=0,content10="Other")) #Rating = 6.59145

#Predicting Robin Hood
predict(newest, data.frame(duration_mins=116, director_column10="Other", action = 1, thriller=0,romance=0,horror=1,drama = 0, animation=0, distributor10="Lionsgate",number_of_faces_in_movie_poster=6, movie_facebook_likes=45140,
color="Color",musical=0,content10="PG-13")) #Rating = 5.87064

#Predicting Creed 2
predict(newest, data.frame(duration_mins=90, director_column10="Robert Zemeckis", action = 0, thriller=0,romance=0,horror=0,drama = 1, animation=0, distributor10="Universal Pictures",number_of_faces_in_movie_poster=1, movie_facebook_likes=28000,
color="Color",musical=0,content10="PG-13")) #Rating = 6.967006

#Predicting The Welcome to Warwen
predict(newest, data.frame(duration_mins=133, director_column10="Other", action = 0, thriller=0,romance=0,horror=0,drama = 1, animation=0, distributor10="Warner Bros.",number_of_faces_in_movie_poster=1, movie_facebook_likes=1000,
color="Color",musical=0,content10="PG-13"))

#Predicting Ralph Breaks the Internet
predict(newest, data.frame(duration_mins=112, director_column10="Other", action = 0, thriller=0,romance=0,horror=0,drama = 0, animation=1, distributor10="Walt Disney Studios Motion Pictures",number_of_faces_in_movie_poster= 78, movie_facebook_likes=1200,
color="Color",musical=0,content10="PG")) #Rating = 3.359926
#seems like the 78 faces is fucking this up.

#Predicting Second Act
predict(newest, data.frame(duration_mins=105, director_column10= "Other", action = 0, thriller=0,romance=1,horror=0,drama = 0, animation=0, distributor10="Other",number_of_faces_in_movie_poster= 1, movie_facebook_likes=47000,color="Color",musical=0,content10="PG-13")) #Rating = 6.707748

#Predicting Becoming Astrid
predict(newest, data.frame(duration_mins=124, director_column10="Other", action = 0, thriller=0,romance=0,horror=0, drama = 1, animation=0, distributor10="Other",number_of_faces_in_movie_poster=2, movie_facebook_likes=0,
color="Color",musical=0,content10="Other")) #Rating = 7.10222
